...
export const getAmountOfKeys = async (address, use) => {  
    try {
        if(use=="legendary"){
            console.log("here1 ", use)
            contractAddress = contractAddressLegend;
        } 
        else if(use=="epic"){
            console.log("here2 ", use)
            contractAddress = contractAddressEpic;
        }
        else if(use=="rare"){
            console.log("her3e ", use)
            contractAddress = contractAddressRare;
        }
        ...
        const res = await PCFContractRead.walletOfOwner(address)
        return parseInt(Number(res.length))
    }
    catch(err){
        console.log(err)
        return 0
...
export const burnKeys = async (address, use) => {  
    try {
        if(use=="legendary"){
            contractAddress = contractAddressLegend;
        } 
        ...
        resBefore = resBefore.map(x => parseInt(Number(x)))
        console.log(resBefore.slice(0, 5))
